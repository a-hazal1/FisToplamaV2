...
async function getCv() {
  ...
  cvReadyPromise = (async () => {
    // GitHub Pages alt-dizinlerinde relative importScripts yolu bazı
    // tarayıcılarda yanlış çözülebiliyor. Worker'ın kendi URL'sinden
    // mutlak OpenCV URL'sini üret.
    const opencvUrl = new URL('opencv.js', self.location.href).href;
    self.postMessage({ id: 0, type: 'runtime-debug', stage: 'opencv-import', url: opencvUrl });
    ...
    while (Date.now() - started < 30000) {
      ...
      // Bazı OpenCV.js build'leri cv'yi Module olarak yayınlar ve
      // runtime initialization callback'inden sonra Mat kullanılabilir olur.
      if (self.Module && self.Module.calledRun && self.cv && typeof self.cv.Mat === 'function') {
        return self.cv;
      }
...
self.onmessage = async function (event) {
  ...
  function stage(name) {
    try { self.postMessage({ id: id, type: 'stage', stage: name, elapsedMs: Date.now() - scanStarted }); } catch (_) {}
  }

  try {
    stage('worker-received');
    stage('opencv-loading');
    ...
    try {
      ...
      stage('fast-detect-success');
    } catch (fastError) {
      stage('fast-detect-failed-' + (fastError && fastError.message ? fastError.message : 'unknown'));
