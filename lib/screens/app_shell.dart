...
class _ReceiptScanPanelState extends State<ReceiptScanPanel> {
  ...
  Future<void> pickMultiple() async {
    final files=await picker.pickMultiImage(imageQuality:95,maxWidth:3000);
  ...
  Future<void> pickCamera() async {
    final file=await picker.pickImage(source:ImageSource.camera,imageQuality:95,maxWidth:3000);
    if(file==null)return;
    await addAndProcess(file);
  }
  ...
  Future<void> saveAll() async {
    ...
    try{
      for(final item in ready){
        ...
        await c.storage.from('receipts').uploadBinary(op,item.originalBytes,fileOptions:const FileOptions(contentType:'image/jpeg'));
        await c.storage.from('receipts').uploadBinary(sp,item.scannedBytes!,fileOptions:const FileOptions(contentType:'image/jpeg'));
